	...
	private void findTheDiamond() {
		...
		for (int i = 0; i < theMurder.size(); i++) {
			for (int j = 0; j < theMurder.get(i).getStomachContents().size(); j++) {
				ArrayList<String> crowStomachContents = theMurder.get(i).getStomachContents();
				if (crowStomachContents.get(j).equals("diamond")) {
					System.out.println(theMurder.get(i).getName());
					x = i;
				}
			}
		}
		System.out.println(x + " innocent crow(s) had to die before the diamond was found");
